...
module.exports = {
    login:  async(req, res) => {
      ...
      //  res.append('Set-Cookie', 'access-token=' + token + ';');
            ...
            return res.json({status:true,login:true ,username:true,password:true,error:false, token: token, user : {name: loginuser.name, phone: loginuser.phone, role: loginuser.role}});
          } else{
